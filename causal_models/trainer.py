...
def preprocess_batch(args, batch, expand_pa=False):
    ...
    if expand_pa:  # used for HVAE parent concatenation
        batch["pa"] = batch["pa"][..., None, None].repeat(1, 1, *tuple(args.input_res))
    for k in batch.keys():
        if (
...
def trainer(args, model, ema, dataloaders, optimizer, scheduler, writer, logger):
    ...
    cr_transf = transforms.Compose(
        [transforms.RandomAffine(degrees=5, 
                                 translate=(0.05,0.05), 
                                 scale=(0.9,1.1)),
        ]) if args.lambda_cr else None

    def run_epoch(dataloader, viz_batch=None, training=True):
        model.train(training)
        model.zero_grad(set_to_none=True)
        stats = {k: 0 for k in ["elbo", "nll", "kl", "n", "nll_aug", "kl_aug", "cr_kl"]} if args.lambda_cr else {k: 0 for k in ["elbo", "nll", "kl", "n"]}
        updates_skipped = 0
        ...
        for i, batch in loader:
            ...
            if training:
                ...
                out = model(batch["x"], batch["pa"], beta=args.beta, cr_transf=cr_transf)
                ...
                if i % args.accu_steps == 0:  # gradient accumulation update
                    ...
                    if args.lambda_cr: # if we apply consistency regularisation
                        nll_aug_nan = torch.isnan(out["nll_aug"]).sum()
                        kl_aug_nan = torch.isnan(out["kl_aug"]).sum()
                        cr_kl_nan = torch.isnan(out["cr_kl"]).sum()
                        if grad_norm < args.grad_skip and nll_nan == 0 and kl_nan == 0 and nll_aug_nan==0 and kl_aug_nan==0 and cr_kl_nan==0:
                            optimizer.step()
                            scheduler.step()
                            ema.update()
                        else:
                            ...
                            logger.info(
                                ...
                                + f" - nll_aug_nan: {nll_aug_nan.item(())} - kl_aug_nan: {kl_aug_nan.item()}"
                                + f" - cr_kl_nan: {cr_kl_nan.item()}"
                            )
                    else:
                        if (grad_norm < args.grad_skip and nll_nan == 0 and kl_nan == 0) or args.iter<500: # Need this as early grad_norm very unstable
                        # if True:
                            optimizer.step()
                            scheduler.step()
                            ema.update()
            ...
            with torch.no_grad():
                ema.ema_model.train(False)
                out = ema.ema_model(batch["x"], batch["pa"], beta=args.beta, cr_transf=cr_transf)
            ...
            if args.lambda_cr:
                stats["nll_aug"] += out["nll_aug"].detach().item() * bs
                stats["kl_aug"] += out["kl_aug"].detach().item() * bs
                stats["cr_kl"] += out["cr_kl"].detach().item() * bs
            ...
            if args.lambda_cr:
                loader.set_description(
                    ...
                    + f' - nll_aug: {stats["nll_aug"] / stats["n"]:.5f}'
                    + f' - kl: {stats["kl"] / stats["n"]:.3f}'
                    + f' - kl_aug: {stats["kl_aug"] / stats["n"]:.3f}'
                    + f' - cr_kl: {stats["cr_kl"] / stats["n"]:.3f}'
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (args.epoch - 1) % args.eval_freq == 0:
            ...
            # reduce_scheduler.step(valid_stats["elbo"])
