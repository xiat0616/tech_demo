...
def plot_counterfactual_viz_mimic(args, x, cf_x, pa, cf_pa, do, rec_loc, save=True):
    ...
    _, _ = plot_cxr_grid(cf_x, ax=ax[1])
    # _, _ = plot_cxr_grid(
    #     rec_loc - x,
    #     ax=ax[3],
    #     fig=fig,
    ...
    for j in range(n):
        ...
        ax[1, j].set_title(rf"do(${msg}$)", fontsize=fs - 2, pad=8)
...
def plot_counterfactual_viz_embed(args, x, cf_x, pa, cf_pa, do, rec_loc, save=True):
    ...
    for j in range(n):
        ...
        for i, (k, v) in enumerate(do.items()):
            if k == "scanner":
                ...
                if isinstance(v[j], str):
                    ...
                    vv = density_categories[int(torch.argmax(v[j], dim=-1))]
                kk = "ds"
            elif k== "view":
                if isinstance(v[j], str):
                    vv = v[j]
                else:
                    vv = view_categories[int(v[j])]
                kk = "vw"
            elif k == 'sex':
                if isinstance(v[j], str):
                    vv = v[j]
                ...
                if isinstance(v[j], str):
                    ...
                    vv = finding_categories_[int(v[j].item())]
                kk = 'd'
            else:
                kk = k
                vv = str(v[j])
        ...
        if "scanner" in pa.keys():
            ...
            title += f"scn={scan}\n"
        if "density" in pa.keys():
            density = density_categories[int(torch.argmax(pa["density"][j], dim=-1))]        
            title += f"density={density}\n"
        if "view" in pa.keys():
            view = view_categories[int(pa["view"][j])]
            title += f"view={view}\n"
        # For MIMIC
        if "finding" in pa.keys():
            finding = str(finding_categories[int(pa['finding'][j].item())])
            title +=f"finding={finding}\n"
        if "sex" in pa.keys():
            sex = str(sex_categories[int(pa['sex'][j].item())])
            title +=f"sex={sex}\n"
        if "race" in pa.keys():
            race = str(race_categories[int(torch.argmax(pa['race'][j], dim=-1))])
            title +=f"race={race}\n"
        if "age" in pa.keys():
            age = str(int(pa['age'][j].item()))
            title +=f"age={age}\n"
        # For MIMIC crop seg
...
def write_images(args, model, batch):
    ...
    if "mimic" in args.hps:
        ...
        try:
            ...
            cf_pa["Left-Lung_volume"] = 1-cf_pa["Left-Lung_volume"]
            do = {"Left-Lung_volume": cf_pa["Left-Lung_volume"]}
