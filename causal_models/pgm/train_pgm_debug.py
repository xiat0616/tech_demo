...
def eval_epoch(model, dataloader):
    ...
    for k in model.variables.keys():
        targets_k = targets[k]  # Use the pre-collected targets
        preds_k = preds[k]

        if k in ['sex', 'finding', 'scanner']:
            stats[k + '_acc'] = (targets_k.squeeze(-1) == torch.round(preds_k)).sum().item() / targets_k.shape[0]
            stats[k + '_rocauc'] = roc_auc_score(targets_k.numpy(), preds_k.numpy(), average='macro')
        elif k in ['age']:
            preds_k = preds_k.unsqueeze(-1)
            assert targets_k.size() == preds_k.size(), f"{k} size doesn't match, targets {targets_k.size()} preds {preds_k.size()}"
            stats[k] = torch.mean(torch.abs(targets_k - preds_k)).item() * 100
        elif k in ['Left-Lung_volume', 'Right-Lung_volume', 'Heart_volume']:
            preds_k = preds_k.unsqueeze(-1)
            assert targets_k.size() == preds_k.size(), f"{k} size doesn't match, targets {targets_k.size()} preds {preds_k.size()}"
            stats[k] = torch.mean(torch.abs(targets_k - preds_k)).item()
        elif k == 'race':
            num_corrects = (targets_k.argmax(-1) == preds_k.argmax(-1)).sum()
            stats[k + "_acc"] = num_corrects.item() / targets_k.shape[0]
            stats[k + "_rocauc"] = roc_auc_score(
                targets_k.numpy(), preds_k.numpy(), multi_class="ovr", average="macro")
...
if __name__ == '__main__':
    ...
    # if "segmentation" in args.exp_name:
    ...
    if not args.testing:
        ...
        for epoch in range(args.epochs):
            ...
            if args.setup in ['sup_pgm', 'sup_aux', 'sup_determ']:
                stats = sup_epoch(
                    model, ema,
                    dataloaders['train'],
                    elbo_fn,
